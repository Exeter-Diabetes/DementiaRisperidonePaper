...
######## This script is used to for the whole analysis, including sensitivity analysis (except discontinuation which has its own script). 
######## The script reads the matched cohort data from the server and defines the outcome and subgroups moving to the modelling part of the study
# # Initialize CPRDData
...
outcomes <- c("Stroke", "stroke.hes") #, "VTE", "cvd", "tia", "heartfailure", "myocardialinfarction", "angina", "pad", "falls", "lowerlimbfracture" )
...
ischaemicstroke <- dat %>% filter(Composite_stroke_type_pre_index == "ischaemic" )
haemorrhagicstroke <- dat %>% filter(Composite_stroke_type_pre_index == "haemorrhagic")
