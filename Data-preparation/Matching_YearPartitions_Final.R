...
sink("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedData_smoking_Alcohol_Inclusive/summary_output.txt", append = TRUE)
...
for (year in 2004:2023) {
    ...
    if (tolower(i) == "stroke") {
      ...
      pre_index_date <- combined_data %>%
        ...
        summarise(
          ...
          pre_index_date_stroke_type_earliest = stroke_cat[which.min(!!sym(obsdate_var))],
          pre_index_date_stroke_type_latest = stroke_cat[which.max(!!sym(obsdate_var))]
        ) %>%
        ungroup()
      
      # Post-index stroke details
      post_index_date <- combined_data %>%
        filter(!!sym(obsdate_var) > issuedate) %>%
        group_by(patid) %>%
        summarise(
          !!post_index_date_date_variable := min(!!sym(obsdate_var), na.rm = TRUE),
          post_index_date_stroke_type = stroke_cat[which.min(!!sym(obsdate_var))]
        ) %>%
        ungroup()
      ...
      combined_data <- combined_data %>%
        ...
        mutate(
          ...
          pre_index_date_stroke_type = pre_index_date_stroke_type_earliest
        ) %>%
        ...
        mutate(
          ...
          post_index_date_stroke_type = post_index_date_stroke_type
        ) %>%
        distinct(patid, .keep_all = TRUE)
    } else {
      combined_data <- combined_data %>%
  ...
  combined_data <- result2 %>%
    left_join(pre_index_date, by = "patid") %>%
    mutate(
      # Pre-index variables for HES
      HES_pre_index_date_stroke = ifelse(!is.na(HES_pre_index_date_earliest_stroke), 1, 0),
      HES_pre_index_date_haemorrhagic = ifelse(!is.na(HES_pre_index_date_earliest_haemorrhagic), 1, 0),
      HES_pre_index_date_ischaemics = ifelse(!is.na(HES_pre_index_date_earliest_ischaemics), 1, 0),
      
      # CPRD stroke types (pre-index)
      cprd_stroke_type_ischaemic = ifelse(pre_index_date_stroke_type == "ischaemic", "ischaemic", NA_character_),
      cprd_stroke_type_ischaemic_date = ifelse(pre_index_date_stroke_type == "ischaemic", as.Date(obsdate_stroke), NA_Date_),
      cprd_stroke_type_haemorrhagic = ifelse(pre_index_date_stroke_type == "haemorrhagic", "haemorrhagic", NA_character_),
      cprd_stroke_type_haemorrhagic_date = ifelse(pre_index_date_stroke_type == "haemorrhagic", as.Date(obsdate_stroke), NA_Date_),
      
      # Unified composite stroke type - Pre-index (combine CPRD and HES)
      Composite_stroke_type_pre_index = case_when(
        !is.na(cprd_stroke_type_ischaemic_date) & 
          (is.na(HES_pre_index_date_earliest_ischaemics) | 
             cprd_stroke_type_ischaemic_date <= HES_pre_index_date_earliest_ischaemics) ~ "ischaemic",
        
        !is.na(HES_pre_index_date_earliest_ischaemics) & 
          (is.na(cprd_stroke_type_ischaemic_date) | 
             HES_pre_index_date_earliest_ischaemics < cprd_stroke_type_ischaemic_date) ~ "ischaemic",
        
        !is.na(cprd_stroke_type_haemorrhagic_date) & 
          (is.na(HES_pre_index_date_earliest_haemorrhagic) | 
             cprd_stroke_type_haemorrhagic_date <= HES_pre_index_date_earliest_haemorrhagic) ~ "haemorrhagic",
        
        !is.na(HES_pre_index_date_earliest_haemorrhagic) & 
          (is.na(cprd_stroke_type_haemorrhagic_date) | 
             HES_pre_index_date_earliest_haemorrhagic < cprd_stroke_type_haemorrhagic_date) ~ "haemorrhagic",
        
        TRUE ~ NA_character_
      ),
      
      Composite_stroke_date_pre_index = pmin(
        as.Date(cprd_stroke_type_ischaemic_date),
        HES_pre_index_date_earliest_ischaemics,
        as.Date(cprd_stroke_type_haemorrhagic_date),
        HES_pre_index_date_earliest_haemorrhagic,
        na.rm = TRUE
      )
    ) %>%
    left_join(post_index_date, by = "patid") %>%
    mutate(
      # Post-index variables for HES
      HES_post_index_date_stroke = ifelse(!is.na(HES_post_index_date_first_stroke), 1, 0),
      HES_post_index_date_haemorrhagic = ifelse(!is.na(HES_post_index_date_first_haemorrhagic), 1, 0),
      HES_post_index_date_ischaemics = ifelse(!is.na(HES_post_index_date_first_ischaemics), 1, 0),
      
      # CPRD stroke types (post-index)
      cprd_stroke_type_ischaemic_post = ifelse(post_index_date_stroke_type == "ischaemic", "ischaemic", NA_character_),
      cprd_stroke_type_ischaemic_post_date = ifelse(post_index_date_stroke_type == "ischaemic", as.Date(obsdate_stroke), NA_Date_),
      cprd_stroke_type_haemorrhagic_post = ifelse(post_index_date_stroke_type == "haemorrhagic", "haemorrhagic", NA_character_),
      cprd_stroke_type_haemorrhagic_post_date = ifelse(post_index_date_stroke_type == "haemorrhagic", as.Date(obsdate_stroke), NA_Date_),
      
      # Unified composite stroke type - Post-index (combine CPRD and HES)
      Composite_stroke_type_post_index = case_when(
        !is.na(cprd_stroke_type_ischaemic_post_date) & 
          (is.na(HES_post_index_date_first_ischaemics) | 
             cprd_stroke_type_ischaemic_post_date <= HES_post_index_date_first_ischaemics) ~ "ischaemic",
        
        !is.na(HES_post_index_date_first_ischaemics) & 
          (is.na(cprd_stroke_type_ischaemic_post_date) | 
             HES_post_index_date_first_ischaemics < cprd_stroke_type_ischaemic_post_date) ~ "ischaemic",
        
        !is.na(cprd_stroke_type_haemorrhagic_post_date) & 
          (is.na(HES_post_index_date_first_haemorrhagic) | 
             cprd_stroke_type_haemorrhagic_post_date <= HES_post_index_date_first_haemorrhagic) ~ "haemorrhagic",
        
        !is.na(HES_post_index_date_first_haemorrhagic) & 
          (is.na(cprd_stroke_type_haemorrhagic_post_date) | 
             HES_post_index_date_first_haemorrhagic < cprd_stroke_type_haemorrhagic_post_date) ~ "haemorrhagic",
        
        TRUE ~ NA_character_
      ),
      
      Composite_stroke_date_post_index = pmin(
        as.Date(cprd_stroke_type_ischaemic_post_date),
        HES_post_index_date_first_ischaemics,
        as.Date(cprd_stroke_type_haemorrhagic_post_date),
        HES_post_index_date_first_haemorrhagic,
        na.rm = TRUE
      )
    )  %>%
  ...
  FullData <- FullData %>%
    ...
    distinct(patid, .keep_all = TRUE) %>%
    left_join(analysis_at_diag$cached(name = "alcohol") %>%
                collect(), by = "patid") %>%
    left_join(analysis_at_diag$cached(name = "smoking") %>% collect(), by = "patid") %>%
    mutate(
      smoking_cat =ifelse(is.na(smoking_cat), "Missing", smoking_cat),
      alcohol_cat =ifelse(is.na(alcohol_cat), "Missing", alcohol_cat))
  ...
  ################################# Matching #########################################
  # 
  m.out2 <- matchit(risperidone ~ + sex + care_home_before_indexdate + deprivation + age_risperidone +pre_index_date_angina + pre_index_date_cvd + 
                      pre_index_date_VTE + pre_index_date_heartfailure + BMI + Prescribed_other_antipsychotic_Prior  + period_before_prescription  +
                      pre_index_date_myocardialinfarction + stroke_composite_pre_index_date + pre_index_date_tia + pre_index_date_falls +
                      pre_index_date_lowerlimbfracture + pre_index_date_ihd + pre_index_date_pad + pre_index_date_af + pre_index_date_revasc + 
                      pre_index_date_qof_diabetes + pre_index_date_anxiety_disorders+ pre_index_date_fh_diabetes + pre_index_date_fh_premature_cvd + 
                      pre_index_date_pulmonary_embolism + pre_index_date_deep_vein_thrombosis + pre_index_date_haem_cancer + pre_index_date_solid_cancer +
                      pre_index_date_hearing_loss + ethnicity + comorbidity_hypertension + totalcholesterol + age_risperidone_cat + alcohol_cat + smoking_cat, data = FullData,
                    exact = ~sex + stroke_composite_pre_index_date + age_risperidone_cat + pre_index_date_cvd, 
  ...
  write.table(match.t, file = paste0("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedData_smoking_Alcohol_Inclusive/match_t_", year, ".txt"), sep = "\t", row.names = FALSE)
  # Save matched data to the list
  file_name <- paste0("C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedData_smoking_Alcohol_Inclusive/matched_data_iteration_", year, ".txt")
...
write.table(all_matched_data, "C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedData_smoking_Alcohol_Inclusive/Joshua_all_matched_data.txt", sep = "\t", row.names = FALSE)
...
write.table(cumulative_matched_data, file = "C:/Users/njc232/OneDrive - University of Exeter/Documents/MatchedData_smoking_Alcohol_Inclusive/Joshua_cumulative_matched_data.txt",sep = "\t", row.names = FALSE)
