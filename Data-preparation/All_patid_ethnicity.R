...
## OF NOTE: the full dataset currently does not have HES linkage, therefore these have been blanked out for now
...
# Observation <- Observation %>% analysis$cached("earlist_obs_other_antipsychotic_pres")
...
gp_5cat_ethnicity <- raw_gp_ethnicity %>% 
  ...
  ungroup() %>% 
  rename(gp_5cat_date = latest_date_per_cat) %>% 
  select(patid, gp_5cat_ethnicity=ethnicity_5cat_cat, gp_5cat_date) %>%
...
gp_16cat_ethnicity <- raw_gp_ethnicity %>% 
  ...
  filter(n()==1) %>%                                                                # keep if 1 row per person
  ungroup() %>% 
  rename(gp_16cat_date = latest_date_per_cat) %>% 
  select(patid, gp_16cat_ethnicity=ethnicity_16cat_cat, gp_16cat_date) %>%
...
gp_qrisk2_ethnicity <- raw_gp_ethnicity %>% 
  ...
  filter(n()==1) %>%                                                                # keep if 1 row per person
  ungroup() %>% 
  rename(gp_qrisk2cat_date = latest_date_per_cat) %>% 
  select(patid, gp_qrisk2_ethnicity=qrisk2_ethnicity_cat, gp_qrisk2cat_date) %>%
...
ethnicity <- cprd$tables$patient %>%
  ...
  select(patid, gp_5cat_ethnicity, gp_16cat_ethnicity, gp_qrisk2_ethnicity) %>%
  
  analysis$cached("gp_ethnicity", unique_indexes="patid")
